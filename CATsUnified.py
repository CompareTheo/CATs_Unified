...
def load_match_list(file_path):
    """Loads a list of values from a file."""
    with open(file_path, 'r') as f:
        return [line.strip() for line in f.readlines()]

# Load the list once at the beginning of the program
match_list = load_match_list('values.txt')
...
def main(*args):
    ...
    for line in lines[1:]:
        ...
        (iz_isbn_query_resp, 
         iz_title_query_resp)= sru.searches(urls, 2)
        ...
        iz_title = sru.parse(iz_title_query_resp, zone="IZ", 
                               inst_code=config.inst_code)
        subfield_0_value = None
        cats_ready = False  # Default to False if no records are found
        if iz_title.numberOfRecords > 0:
            _, cats_ready, subfield_0_value = sru.get_e_holdings(iz_title.records, zone="IZ", 
                                       inst_code=config.inst_code, 
        ...
        permalink_creation = f"https://lmu.primo.exlibrisgroup.com/permalink/01LMU_INST/mq4q04/alma{subfield_0_value}"
        subfield_0_display = permalink_creation if cats_ready else ""
        cats_ready_flag = "X" if cats_ready else ""
        ...
        if order_options_found == "X" and iz_title_recs_found != "X" and iz_isbn_recs_found !="X":
            tag = "ok_to_order"
        ...
        gui.insert_text(gui.counter, (order.isbn, order.title.title(), order.author.title(), 
              ...
              iz_isbn_recs_found, iz_title_recs_found, cats_ready_flag, 
              results, subfield_0_display), tag)
...
class gui:
    def __init__(self, master):
        ...
        master.minsize(width=1350, height=900)
        master.maxsize(width=1350, height=900)
        ...
        self.tree['columns'] = ('isbn', 'title', 'author', 'pub_date', 
                        ...
                        'iz_search_title', 'cats_ready', 'gobipurchase', 
                        'permalink')
        ...
        self.tree.heading('cats_ready', text="CATs Ready", anchor="center")
        ...
        self.tree.column("cats_ready", width=70, anchor="center")
        self.tree.column("gobipurchase", width=250, anchor="center")
        ...
        self.tree.tag_configure('cats_ready', background='#ecf0f1')
    ...
    def save_log_xlsx(self):
        ...
        # save the file
        wb.save(f"{config.log_directory}/CATs_Ready_log.xlsx")
        self.msgbox("LOG SAVED SUCCESFULLY.")
